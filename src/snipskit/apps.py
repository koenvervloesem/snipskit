...
- By subclassing :class:`HermesSnipsApp`: This creates a Snips app using the
  Hermes protocol.
- By subclassing :class:`MQTTSnipsApp`: This creates a Snips app using the MQTT
  protocol directly.

:class:`HermesSnipsApp` is a subclass of :class:`HermesSnipsComponent` and adds
    ...
    def __init__(self, snips=None, config=None):
        ...
        Args:
            snips (:class:`SnipsConfig`, optional): a Snips configuration.
                If the argument is not specified, a default :class:`SnipsConfig`
                object is created for a locally installed instance of Snips.

            config (:class:`AppConfig`, optional): an app configuration. If the
